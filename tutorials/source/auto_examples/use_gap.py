...
Using GAP on test data
======================

In this tutorial you will learn to use the trained potential on test data. 
...
ml_potential = Potential(param_filename=(PROJECT_PATH / "gap/SOAP_500.xml"))
...
# %%
# To evaluate the ML potential, we will compare predicted energies (and forces) with DFT energies (and forces). To do so, we will load the coordinates, energies and forces from the DFT simulation. Then we will predict the energy for given coordinates using the ML potential, and compare with the reference energies from the DFT simulation.
...
# %%
# Finally, we plot the error and force correlation plots for the training and the validation data.
